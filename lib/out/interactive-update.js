...
function createChoices(packages, options) {
    ...
    const choicesWithTableFormating = _.map(choices, (choiceEl, i) => {
        choiceEl.name = choicesAsATable[i];
        return choiceEl;
    });
...
function interactive(currentState) {
    ...
    return new Promise(resolve => inquirer.prompt(questions, resolve)).then(answers => {
        ...
        return installPackages(saveDependencies, currentState)
            .then(curState => installPackages(saveDevDependencies, curState))
            .then(curState => {
                console.log('');
                console.log(chalk.green(`[yarn-check] Update complete!`));
                console.log(chalk.green('[yarn-check] ' + updatedPackages));
                console.log(chalk.green(`[yarn-check] You should re-run your tests to make sure everything works with the updates.`));
                return curState;
            });
    });
}
